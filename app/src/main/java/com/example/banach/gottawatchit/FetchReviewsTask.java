...
public class FetchReviewsTask extends AsyncTask<String, Void, ArrayList<Review>>{
    ...
    FetchReviewsTask(ReviewCallback ReviewCallback) {
        mReviewCallback = ReviewCallback;
            }

    @Override
    protected void onPostExecute(ArrayList<Review> items) {
        mReviewCallback.onTaskDone(list);
        }
    ...
    protected ArrayList<Review> doInBackground(String... params) {
        list.clear();
            String search = "http://api.themoviedb.org/3/movie/" + params[0] + "/reviews?api_key=" + myKey;
        ...
        try {
            ...
            if (featureArray.length() > 0) {
                ...
                for (int i = 0; i < featureArray.length(); i++) {
                    ...
                    String rev = firstFeature.getString("content");
                    String author = firstFeature.getString("author");
                    list.add(new Review(author,rev));
                }
            }
